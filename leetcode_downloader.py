...
code_links = os.path.join(root_path, "code_links")
# Default Directory for chrome downloads
download_default_directory = os.path.join(root_path, "chrome_download")
...
all_file_dir = [accepted_dir, wrong_answer_dir,
                tle_dir, runtime_error_dir, compile_error_dir]
...
if acc_type == "leetcode":
    already_present = driver.find_element_by_name(
        "login").get_attribute('value')
    ...
    try:
        err_msg = driver.find_element_by_xpath(
            "/html/body/div[1]/div/div[2]/div/div[2]/div/div/div/p").text
        if "CAPTCHA" in err_msg:
            ...
            driver.find_element_by_xpath(
                "//button[@class='btn__2FMG fancy-btn__CYhs primary__3S2m light__3zR9 btn__1eiM btn-md__3VAX ']").click()
            # You may increase below timer to 10 if you have slow internet connection.
            time.sleep(5)
            err_msg = driver.find_element_by_xpath(
                "/html/body/div[1]/div/div[2]/div/div[2]/div/div/div/p").text
    ...
    driver.find_element_by_xpath(
        "/html/body/div[1]/div/div[2]/div/div[2]/div/div/div/div[2]/div/a[1]").click()
    time.sleep(5)
    linkedin_username_xpath = "/html/body/div/main/div/form/div[1]/input"

    already_present = driver.find_element_by_xpath(
        linkedin_username_xpath).get_attribute('value')
    for i in range(len(already_present)+1):
        driver.find_element_by_xpath(
            linkedin_username_xpath).send_keys(Keys.BACKSPACE)
    # user name will be entered
    ...
    already_present = driver.find_element_by_xpath(
        linkedin_password_xpath).get_attribute('value')
    for i in range(len(already_present)+1):
        driver.find_element_by_xpath(
            linkedin_password_xpath).send_keys(Keys.BACKSPACE)
    # password will be entered
    ...
    try:
        ...
        err_linkedin_username = "/html/body/div/main/div/form/div[1]/div"
        err_mes_username = driver.find_element_by_xpath(
            err_linkedin_username).text
    ...
    try:
        # Hmm, that's not the right password   or    The password you provided must have at least 6 characters.
        err_linkedin_password = "/html/body/div/main/div/form/div[2]/div"
        err_mes_password = driver.find_element_by_xpath(
            err_linkedin_password).text
        ...
        try:
            ...
            linkedin_captcha_msg = driver.find_element_by_xpath(
                linkedin_captcha).text
            if len(linkedin_captcha_msg) > 0:
                print(
                    "If captcha issue persist then connect to the VPN and rerun the code")
                print(
                    "                              OR                                   ")
                print(
                    "ENTER THE CAPTCHA AND THEN PRESS 'y' KEY FOLLOWED BY ENTER TO PROCEED : ", end="")
                input()
                print()
        except Exception as e:
        ...
        try:
            ...
            linkedin_allow_text = driver.find_element_by_xpath(
                linkedin_allow).text
    ...
    driver.find_element_by_xpath(
        "/html/body/div[1]/div/div[2]/div/div[2]/div/div/div/div[2]/div/a[2]").click()
    time.sleep(5)
    ...
    already_present = driver.find_element_by_xpath(
        google_username_xpath).get_attribute('value')
    for i in range(len(already_present)+1):
        driver.find_element_by_xpath(
            google_username_xpath).send_keys(Keys.BACKSPACE)
    # user name will be entered
    ...
    try:
        ...
        already_present = driver.find_element_by_xpath(
            google_password_xpath).get_attribute('value')
        for i in range(len(already_present)+1):
            driver.find_element_by_xpath(
                google_password_xpath).send_keys(Keys.BACKSPACE)
        # password will be entered
    ...
    driver.find_element_by_xpath(
        "/html/body/div[1]/div/div[2]/div/div[2]/div/div/div/div[2]/div/a[3]").click()
    time.sleep(5)

    github_username_xpath = "/html/body/div[3]/main/div/form/div[2]/input[1]"

    already_present = driver.find_element_by_xpath(
        github_username_xpath).get_attribute('value')
    for i in range(len(already_present)+1):
        driver.find_element_by_xpath(
            github_username_xpath).send_keys(Keys.BACKSPACE)
    # user name will be entered
    ...
    already_present = driver.find_element_by_xpath(
        github_password_xpath).get_attribute('value')
    for i in range(len(already_present)+1):
        driver.find_element_by_xpath(
            github_password_xpath).send_keys(Keys.BACKSPACE)
    # password will be entered
    ...
    try:
        ...
        err_github_username_or_password = "/html/body/div[3]/main/div/form/div[2]/div/div"
        err_mes_username = driver.find_element_by_xpath(
            err_github_username_or_password).text
        if(len(err_mes_username) > 0):
    ...
    driver.find_element_by_xpath(
        "/html/body/div[1]/div/div[2]/div/div[2]/div/div/div/div[2]/div/a[4]").click()
    time.sleep(5)
    ...
    try:
        ...
        already_present = driver.find_element_by_xpath(
            fb_username_xpath).get_attribute('value')
        for i in range(len(already_present)+1):
            driver.find_element_by_xpath(
                fb_username_xpath).send_keys(Keys.BACKSPACE)
        # user name will be entered
        ...
        already_present = driver.find_element_by_xpath(
            fb_password_xpath).get_attribute('value')
        for i in range(len(already_present)+1):
            driver.find_element_by_xpath(
                fb_password_xpath).send_keys(Keys.BACKSPACE)
        # password will be entered
...
while(True):
    tbody = driver.find_element_by_id(
        "submission-list-app").find_element(By.TAG_NAME, "tbody")
    all_tr = tbody.find_elements(By.TAG_NAME, "tr")
    for row in all_tr:
        # get status column value from table
        third_row_value = row.find_elements(By.TAG_NAME, "td")[2]
        status = third_row_value.find_elements(By.TAG_NAME, "strong")[
            0].text   # extract status from strong field
        submission_link = third_row_value.find_elements(
    ...
    try:
        ...
        next_page = driver.find_elements_by_class_name(
            "next")[0].find_element(By.TAG_NAME, "a").get_attribute('href')
...
for i in range(len(all_file_list)):
    ...
    with open(os.path.join(code_links, file_name)) as f:
        ...
        for submission_link in f:
            ...
            time.sleep(10)
            que_name = driver.find_element(
                By.TAG_NAME, "h4").find_element(By.TAG_NAME, "a").text
            ...
            prog_lang = driver.find_elements_by_id(
                "result_language")[0].text.strip()
